...
const STEPS = [
  ...
  },
  {
    ...
    intro: "현재 시간대 도착. 지구 궤도 진입에는 인증이 필요합니다. 1단계 — 이 행성에서 관측된 현상 4가지가 각각 어느 두 권 사이의 상호작용인지 판단해, 회로를 연결하십시오.",
    hint: "'무엇이 무엇과 물질·에너지를 주고받는가'로 판단하십시오. 화산은 땅(지권)의 물질을 대기(기권)로 내보냅니다. 생물과 대기, 바다와 대기, 물과 땅 사이에서 일어나는 현상도 있습니다.",
...
function WiringStep({ done, wrong, locked }) {
  ...
  // 4가지 현상 — 설명에 권 이름을 직접 쓰지 않고, 무엇이 무엇과 주고받는지 추론하게 함 (화살표 6개 중 4개만 정답 자리)
  const ITEMS = [
    { p: "화산 폭발", d: "화산 폭발로 화산재와 화산 가스가 분출된다.", edge: "지-기" },
    { p: "증산 작용", d: "식물의 증산 작용으로 대기에 수증기가 공급된다.", edge: "기-생" },
    { p: "태풍 발생", d: "따뜻한 바다에서 공급된 열과 수증기로 태풍이 발생한다.", edge: "기-수" },
    { p: "U자곡과 V자곡", d: "빙하와 하천의 침식 작용으로 U자곡과 V자곡이 형성된다.", edge: "지-수" },
  ];
  ...
  const check = () => {
    if (locked) return;
    // 카드 4장이 모두 놓였고, 각 카드가 자기 정답 화살표에 있어야 통과 (빈 화살표 2개는 무관)
    const ok = ITEMS.length === Object.keys(assign).length &&
      Object.entries(assign).every(([e, idx]) => ITEMS[idx].edge === e);
    ok ? done() : wrong();
  };
